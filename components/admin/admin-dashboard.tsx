...
export function AdminDashboard() {
  ...
  const loadUsers = async () => {
    try {
      ...
      const usersData = await RoleAuthService.getAllUsersWithRoles(currentUser.id);
      setUsers(usersData);
    } catch (error: any) {
  ...
  const updateUserRole = async (userId: string, newRole: UserRole) => {
    try {
      ...
      const success = await RoleAuthService.updateUserRole(currentUser.id, userId, newRole);
      if (success) {
  ...
  const getStats = () => {
    ...
    return { totalUsers, adminUsers, moderatorUsers, premiumUsers, regularUsers };
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{stats.moderatorUsers}</div>
          </CardContent>
        </Card>
