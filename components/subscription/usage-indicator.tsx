    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          {limits.max_resumes && (
            <Progress
              value={getUsagePercentage(usage.resumes_created, limits.max_resumes)}
              className={isNearLimit(usage.resumes_created, limits.max_resumes) ? "bg-red-100" : ""}
            />
          )}
        </div>
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-sm">
            ...
            <span>
              ...
              {limits.max_downloads_per_month ? ` / ${limits.max_downloads_per_month}` : " (Unlimited)"}
            </span>
          </div>
          {limits.max_downloads_per_month && (
            <Progress
              value={getUsagePercentage(usage.downloads_used, limits.max_downloads_per_month)}
              className={isNearLimit(usage.downloads_used, limits.max_downloads_per_month) ? "bg-red-100" : ""}
            />
          )}
        </div>
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-sm">
            ...
            <span>
              ...
              {limits.ai_optimizations_per_month ? ` / ${limits.ai_optimizations_per_month}` : " (Unlimited)"}
            </span>
          </div>
          {limits.ai_optimizations_per_month && (
            <Progress
              value={getUsagePercentage(usage.ai_optimizations_used, limits.ai_optimizations_per_month)}
              className={
                isNearLimit(usage.ai_optimizations_used, limits.ai_optimizations_per_month) ? "bg-red-100" : ""
              }
            />
          )}
        </div>
        ...
        {(isNearLimit(usage.resumes_created, limits.max_resumes) ||
          ...
          isNearLimit(usage.ai_optimizations_used, limits.ai_optimizations_per_month)) && (
