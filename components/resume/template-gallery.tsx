...
export function ResumeGallery() {
	...
	useEffect(() => {
		fetch("/api/resume?offset=" + resumes.length)
			.then((res) => res.json())
			.then(setResumes);
	}, []);
	...
	const loadMoreTemplates = () => {
		...
		fetch("/api/resume?offset=" + resumes.length)
			.then((res) => res.json())
			.then((newTemplates) => {
				setResumes((prev) => [...prev, ...newTemplates]);
			});
	};
			...
			<div className="flex items-center justify-between">
				<div>
					...
					<p className="text-gray-600 dark:text-gray-400">
						{resumes.length} professional resumes available
					</p>
				</div>
								...
								<div className="flex items-center justify-between text-sm text-gray-500 dark:text-gray-300">
									<div className="flex items-center gap-4">
										...
										<div className="flex items-center gap-1">
											<Star className="w-4 h-4 fill-yellow-400 text-yellow-400" />
											<span>
												{resume.ai_optimization_score}
											</span>
										</div>
										<div className="flex items-center gap-1">
											<Download className="w-4 h-4" />
											<span>{resume.download_count}</span>
										</div>
									</div>
