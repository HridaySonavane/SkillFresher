...
export function UsageOverview({ usage, subscription }: UsageOverviewProps) {
  ...
  const currentUsage = usage || { resumes_created: 0, downloads_used: 0, ai_optimizations_used: 0 }
      ...
      <CardContent className="space-y-6">
        {usageItems.map((item, index) => (
          <div key={index} className="space-y-2">
            ...
            <Progress value={item.limit === -1 ? 0 : (item.current / item.limit) * 100} className="h-2" />
            {item.limit !== -1 && item.current >= item.limit * 0.8 && (
              <p className="text-xs text-amber-600">You're approaching your {item.label.toLowerCase()} limit</p>
            )}
          </div>
        ))}
        ...
        {!subscription && (
          <div className="mt-6 p-4 bg-blue-50 rounded-lg">
            ...
            <p className="text-sm text-blue-700 mb-3">
              Get unlimited resumes, downloads, and AI optimizations with our premium plans.
            </p>
            <Button size="sm" asChild>
