...
import { ArrowLeft, Save, Download, Share2, Eye, Settings } from "lucide-react"
          ...
          {templateName && currentStep !== "template" && (
            <>
              ...
              <div className="text-sm">
                ...
                <span className="font-medium text-gray-900 ml-1">{templateName}</span>
              </div>
            </>
          )}
        </div>
      ...
      <div className="mt-4">
        <div className="flex items-center justify-center">
          <div className="flex items-center space-x-4">
            {[
              { step: 1, label: "Template", active: currentStep === "template", completed: ["form", "preview"].includes(currentStep) },
              { step: 2, label: "Details", active: currentStep === "form", completed: ["preview"].includes(currentStep) },
              { step: 3, label: "Preview", active: currentStep === "preview", completed: false }
            ].map((item, index) => (
              <div key={item.step} className="flex items-center">
                ...
                <span className={`ml-2 text-sm font-medium ${
                  item.active ? 'text-blue-600' : item.completed ? 'text-green-600' : 'text-gray-500'
                }`}>
                  {item.label}
                </span>
                {index < 2 && (
