        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2 text-sm">
              <div className={`p-2 rounded ${hasRole(userRole, 'user') ? 'bg-green-100' : 'bg-gray-100'}`}>
                User Access: {hasRole(userRole, 'user') ? '✅' : '❌'}
              </div>
              <div className={`p-2 rounded ${hasRole(userRole, 'premium_user') ? 'bg-green-100' : 'bg-gray-100'}`}>
                Premium Access: {hasRole(userRole, 'premium_user') ? '✅' : '❌'}
              </div>
              <div className={`p-2 rounded ${hasRole(userRole, 'moderator') ? 'bg-green-100' : 'bg-gray-100'}`}>
                Moderator Access: {hasRole(userRole, 'moderator') ? '✅' : '❌'}
              </div>
              <div className={`p-2 rounded ${hasRole(userRole, 'admin') ? 'bg-green-100' : 'bg-gray-100'}`}>
          ...
          {userProfile && (
            <div className="space-y-2">
              ...
              <div className="text-sm space-y-1">
                ...
                <div><strong>Role:</strong> {getRoleDisplayName(userProfile.role)}</div>
                <div><strong>Subscription Tier:</strong> {userProfile.subscription_tier}</div>
                <div><strong>Created:</strong> {new Date(userProfile.created_at).toLocaleDateString()}</div>
              </div>
            </div>
          )}
          ...
          <div className="space-y-2">
            ...
            {hasRole(userRole, 'admin') && (
              <div className="p-3 bg-red-50 border border-red-200 rounded">
                <strong>Admin Panel:</strong> You have full administrative access.
              </div>
            )}
            ...
            {hasRole(userRole, 'premium_user') && (
              <div className="p-3 bg-purple-50 border border-purple-200 rounded">
                <strong>Premium Features:</strong> You have access to premium templates and features.
              </div>
            )}
            
            <div className="p-3 bg-blue-50 border border-blue-200 rounded">
              <strong>Basic Features:</strong> All users have access to basic resume creation.
            </div>
          </div>
